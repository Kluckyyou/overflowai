# OverflowAI Deployment Files

This directory contains files for deploying OverflowAI on Linux servers.
...
| `overflowai.service` | Systemd service unit file |
| `overflowai-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Kluckyyou/overflowai/main/deploy/docker-deploy.sh | bash

# Or download first, then run
curl -sSL https://raw.githubusercontent.com/Kluckyyou/overflowai/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
# View logs
docker compose -f docker-compose.local.yml logs -f overflowai

# If admin password was auto-generated, find it in logs:
docker compose -f docker-compose.local.yml logs overflowai | grep "admin password"
...
```bash
# Clone repository
git clone https://github.com/Kluckyyou/overflowai.git
cd overflowai/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs overflowai | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/overflowai-datamanagement.sock`
...
# Restart OverflowAI only
docker compose -f docker-compose.local.yml restart overflowai
...
# View logs
docker compose logs -f overflowai

# Restart OverflowAI only
docker compose restart overflowai
...
| `ADMIN_EMAIL` | No | `admin@overflowai.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf overflowai-complete.tar.gz deployment/

# Transfer to new server
scp overflowai-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf overflowai-complete.tar.gz
cd deployment/
docker compose -f docker-compose.local.yml up -d
```
...
OverflowAI supports three methods to connect to Gemini:
...
```bash
curl -sSL https://raw.githubusercontent.com/Kluckyyou/overflowai/main/deploy/install.sh | sudo bash
```

### Manual Installation

1. Download the latest release from [GitHub Releases](https://github.com/Kluckyyou/overflowai/releases)
2. Extract and copy the binary to `/opt/overflowai/`
3. Copy `overflowai.service` to `/etc/systemd/system/`
4. Run:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable overflowai
   sudo systemctl start overflowai
   ```
...
# Stop the service
sudo systemctl stop overflowai

# Restart the service
sudo systemctl restart overflowai

# Check status
sudo systemctl status overflowai

# View logs
sudo journalctl -u overflowai -f
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit overflowai
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/overflowai.service
   ```
...
The main config file is at `/etc/overflowai/config.yaml` (created by Setup Wizard).
...
```
/opt/overflowai/
├── overflowai              # Main binary
├── overflowai.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 overflowai
...
# View detailed logs
docker compose logs --tail=100 overflowai
...
# View recent logs
sudo journalctl -u overflowai -n 50

# Check config file
sudo cat /etc/overflowai/config.yaml
...
OverflowAI supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[your-domain.example](https://your-domain.example/)** to get TLS fingerprint information for different devices and browsers.
