#!/bin/bash
#
# OverflowAI Installation Script
# OverflowAI 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Kluckyyou/overflowai/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Kluckyyou/overflowai"
INSTALL_DIR="/opt/overflowai"
SERVICE_NAME="overflowai"
SERVICE_USER="overflowai"
CONFIG_DIR="/etc/overflowai"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="OverflowAI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 OverflowAI 服务："
    ...
    ["not_installed"]="OverflowAI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 OverflowAI。"
    ...
    ["uninstall_complete"]="OverflowAI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 OverflowAI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 OverflowAI"
    ...
    ["server_config_desc"]="配置 OverflowAI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="OverflowAI Installation Script"
    ...
    # Completion
    ["install_complete"]="OverflowAI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start OverflowAI service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading OverflowAI..."
    ...
    ["not_installed"]="OverflowAI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove OverflowAI from your system."
    ...
    ["uninstall_complete"]="OverflowAI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install OverflowAI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove OverflowAI"
    ...
    ["server_config_desc"]="Configure OverflowAI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/overflowai" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/overflowai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="overflowai_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/overflowai" "$INSTALL_DIR/overflowai"
    chmod +x "$INSTALL_DIR/overflowai"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/overflowai"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/overflowai.service << EOF
[Unit]
Description=OverflowAI - AI API Gateway Platform
Documentation=https://github.com/Kluckyyou/overflowai
...
User=overflowai
Group=overflowai
WorkingDirectory=/opt/overflowai
ExecStart=/opt/overflowai/overflowai
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=overflowai
...
start_service() {
    ...
    if systemctl start overflowai; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u overflowai -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable overflowai 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status overflowai"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u overflowai -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart overflowai"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop overflowai"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if OverflowAI is installed
    if [ ! -f "$INSTALL_DIR/overflowai" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/overflowai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet overflowai; then
        print_info "$(msg 'stopping_service')"
        systemctl stop overflowai
    fi

    # Backup current binary
    cp "$INSTALL_DIR/overflowai" "$INSTALL_DIR/overflowai.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/overflowai.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/overflowai"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start overflowai

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: OverflowAI must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/overflowai" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="overflowai.backup.${current_version}"
        else
            backup_name="overflowai.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/overflowai" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop overflowai 2>/dev/null || true
    systemctl disable overflowai 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/overflowai.service
    systemctl daemon-reload
