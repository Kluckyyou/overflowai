...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/overflowai-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/overflowai-data", "logs", "overflowai.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "overflowai.log")
