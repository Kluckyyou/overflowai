...
const (
	...
	githubRepo     = "Kluckyyou/overflowai"
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".overflowai-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "overflowai")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "overflowai" || baseName == "overflowai.exe" {
