...
OverflowAI has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
- [Migrating from a Legacy External Payment System](#migrating-from-a-legacy-external-payment-system)
...
## Migrating from a Legacy External Payment System

If you previously used a separate external payment service, you can migrate to the built-in payment system:

### Key Differences

| Aspect | Legacy external service | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into OverflowAI, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in OverflowAI admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in OverflowAI admin dashboard |

### Migration Steps

1. Enable payment in OverflowAI admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to OverflowAI's callback endpoints
3. Verify that new orders are processed correctly via built-in payment
4. Decommission the legacy external payment service

> **Note**: Historical order data from the external service will not be automatically migrated. Keep the old service running for a while if you need to access historical records.
